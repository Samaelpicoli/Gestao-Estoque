...
class TestsNewBrandView(TestCase):
    """
    Testes para a view de criação de uma marca na aplicação.
    ...
    def setUp(self):
        ...
        Ele cria um usuário de teste com permissões adequadas para criar
        marcas e faz o login desse usuário.
        """
    ...
    def test_new_view_a_ser_acessado_retorna_status_code_200(self):
        """
        Testa se a página de criação de uma marca está acessível
        e retorna o status code 200.
        """
        esperado = HTTPStatus.OK
        response = self.client.get(reverse('brand_create'))
        self.assertEqual(response.status_code, esperado)

    def test_new_view_template_utilizado_retorna_brand_create_html(self):
        """
        Testa se o template correto ('brand_create.html')
        é utilizado na página de criação de marca.
        """
        esperado = 'brand_create.html'
        response = self.client.get(reverse('brand_create'))
        self.assertTemplateUsed(response, esperado)

    def test_new_view_valido_cadastra_marca_e_retorna_qtd_marcas_e_redirecionamento(  # noqa: E501
        self,
    ):  # noqa: E501
        """
        Testa se, ao enviar um formulário válido, uma marca é cadastrado,
        a quantidade de marcas aumenta para 1 e ocorre o redirecionamento
        para a lista de marcas.
        """
        esperado_status_code = HTTPStatus.FOUND
        esperado_quantidade_marcas = 1
        esperado_redirect = 'brand_list'

        response = self.client.post(
            reverse('brand_create'),
            data={'name': 'Ninja'},
        )

        self.assertEqual(response.status_code, esperado_status_code)
        self.assertTrue(Brand.objects.filter(name='Ninja').exists())
        self.assertEqual(Brand.objects.count(), esperado_quantidade_marcas)
        self.assertRedirects(response, reverse(esperado_redirect))

    def test_new_view_invalido_retorna_invalido_e_zero_marcas(self):
        """
        Testa se, ao enviar um formulário com dados inválidos (sem nome),
        o formulário é invalidado, nenhuma marca é criado e o template
        correto é renderizado.
        """
        esperado_status_code = HTTPStatus.OK
        esperado_template_utilizado = 'brand_create.html'
    ...
    def test_create_view_sem_login_retorna_redirecionameto_a_url_login(self):
        ...
        login quando tenta acessar a view de criação de uma marca.
        """
        esperado_status_code = HTTPStatus.FOUND
        self.client.logout()
        response = self.client.post(reverse('brand_create'))
        self.assertEqual(response.status_code, esperado_status_code)
    ...
    def test_create_view_user_sem_permissao_retorna_bloqueado(self):
        ...
        quando tenta acessar a view de criação de marcas.
        """
