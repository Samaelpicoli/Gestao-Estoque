...
class TestsNewCategoryView(TestCase):
    """
    Testes para a view de criação de uma categoria na aplicação.
    ...
    def setUp(self):
        """
        Configura o ambiente de teste para a View de Criação de Marcas.
        ...
        categorias e faz o login desse usuário.
        """
    ...
    def test_new_view_a_ser_acessado_retorna_status_code_200(self):
        """
        Testa se a página de criação de uma categoria está acessível
    ...
    def test_new_view_template_utilizado_retorna_category_create_html(self):
        """
        Testa se o template correto ('category_create.html')
        é utilizado na página de criação de categoria.
        """
        esperado = 'category_create.html'
        response = self.client.get(reverse('category_create'))
        self.assertTemplateUsed(response, esperado)

    def test_new_view_valido_cadastra_categoria_e_retorna_qtd_categorias_e_redirecionamento(  # noqa: E501
        self,
    ):  # noqa: E501
        """
        Testa se, ao enviar um formulário válido, uma categoria é cadastrado,
        a quantidade de categorias aumenta para 1 e ocorre o redirecionamento
        para a lista de categorias.
        """
        esperado_status_code = HTTPStatus.FOUND
        esperado_quantidade_categorias = 1
        esperado_redirect = 'category_list'
        ...
        self.assertTrue(Category.objects.filter(name='Teclado').exists())
        self.assertEqual(
            Category.objects.count(), esperado_quantidade_categorias
        )
        self.assertRedirects(response, reverse(esperado_redirect))

    def test_new_view_invalido_retorna_invalido_e_zero_categorias(self):
        """
        Testa se, ao enviar um formulário com dados inválidos (sem nome),
        o formulário é invalidado, nenhuma categoria é criado e o template
        correto é renderizado.
        """
        esperado_status_code = HTTPStatus.OK
        esperado_template_utilizado = 'category_create.html'
    ...
    def test_list_view_user_sem_permissao_retorna_bloqueado(self):
        ...
        quando tenta acessar a view de criação de categorias.
        """
